...
greet(null);// -> 개발자가 명시적으로 표현한값
greet(undefined, "누구십니까>");// ->시스템이 정해준 빈값(개발자가 선언 후 시용하지 않은 값)
...
//3. 콜백함수 
//특정 함수를 실행 할 때 실행하는 사람이 특정 기능을 완료한 후에 실행하고싶은 코드를 정의하는 용도
const run = (callback) => {
    callback();
}
...
run(() => {
    console.log("run이 실행됨2222");
})
run(() => {
    console.log("run이 실행됨333");
})