...
/*
    ...
    생명 주기에 따른 메서드를 사용할수 있음.

    state값이 변경된면 리액트는 변화를 감지하고 그에 맞게 컴포넌트를 리렌더링 한다.
    그래서 state값을 변경할 때는 state에 어떤 값을 직접 변경하는 것이 아닌
    this.setState라는 함수를 이용해서 완전히 새로운 state값을 넣어주어야한다.

    react의 componet 라이프사이클은 생성(mount), 수정(update), 제거(unmount) 단계로 나누어
