                ...
                <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden border border-gray-200 animate-[fadeIn_0.2s_ease-out]" onClick={e => e.stopPropagation()}>
                    ...
                    <div className="p-8 space-y-6 max-h-[80vh] overflow-y-auto">
                        ...
                        <div>
                            ...
                            <select 
                                className="w-full border border-gray-200 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:border-black bg-white transition-all shadow-sm"
                                value={newEventData.creatorId}
                                onChange={e => setNewEventData({...newEventData, creatorId: e.target.value})}
                            >
                        ...
                        {newEventData.type === 'joint' && (
                            <div className="p-4 bg-purple-50 rounded-xl border border-purple-100 animate-[slideDown_0.2s_ease-out]">
                                <label className="block text-xs font-bold text-purple-700 mb-3 flex items-center gap-1.5">
                                    <Users size={14} /> 합방 참여 크리에이터 선택 (최소 1명)
                                </label>
                                ...
                                <div className="max-h-40 overflow-y-auto space-y-1.5 pr-1 custom-scrollbar">
                                        ...
                                        >
                                            <div className="flex items-center gap-2">
                                                <div className="w-5 h-5 rounded-full bg-gray-200 overflow-hidden border border-purple-200/50 flex-shrink-0">
                                                    {p.avatarUrl ? <img src={p.avatarUrl} className="w-full h-full object-cover"/> : <UserIcon size={12} className="m-auto text-gray-400"/>}
                                                </div>
                                    ...
                                    {potentialPartners.length === 0 && <div className="text-[10px] text-purple-400 text-center py-4">검색 결과가 없습니다.</div>}
                                </div>
                                
                                {newEventData.partnerCreators.length > 0 && (
                                    <div className="mt-3 flex flex-wrap gap-1.5 pt-3 border-t border-purple-200/50">
                                        <span className="text-[10px] text-purple-500 font-bold uppercase w-full mb-1">선택된 파트너</span>
                                        {newEventData.partnerCreators.map(id => (
                                            <span key={id} className="bg-purple-100 text-purple-700 text-[10px] font-bold px-2 py-0.5 rounded-full flex items-center gap-1">
                                                {creators.find(c => c.id === id)?.name}
                                                <X size={10} className="cursor-pointer" onClick={() => togglePartnerCreator(id)} />
                                            </span>
                                        ))}
                                    </div>
                                )}
                        ...
                        <div className="space-y-4 mb-8">
                            <div className="flex items-center gap-3 bg-gray-50 p-3 rounded-xl border border-gray-100">
                                <div className="w-10 h-10 rounded-full border-2 border-white overflow-hidden shadow-sm flex-shrink-0">
                                    {creatorsMap[selectedEvent.creatorId]?.avatarUrl ? <img src={creatorsMap[selectedEvent.creatorId].avatarUrl} className="w-full h-full object-cover" /> : <UserIcon className="p-2 text-gray-400"/>}
                                </div>
                                <div className="overflow-hidden">
                                        ...
                                        {selectedEvent.partnerCreators.map(pid => (
                                            <div key={pid} className="flex items-center gap-1.5 bg-purple-50 text-purple-700 px-2 py-1 rounded-lg border border-purple-100 text-xs font-medium">
                                                <div className="w-4 h-4 rounded-full bg-purple-200 overflow-hidden">
                                                    {creatorsMap[pid]?.avatarUrl && <img src={creatorsMap[pid].avatarUrl} className="w-full h-full object-cover" />}
                                                </div>
