...
public class SrveStream {
	...
	public void fileRead() {
		//FileReader : 파일을 직접적으로 연결해서 2바이트단위로 입력 받을수 있는 기반 스트림
	 ...
	 * - 자바에서는 클래스를 직렬화 하기 위해 직렬화 대상클래스에게는 implements Serializable 해야 함.
	 * 
	 * 역직렬화
	...
	public void ObjectSave() {
		Product p1 = new Product("아이폰16", 100000);
		Product p2 = new Product("아이폰16pro", 120000);
		Product p3 = new Product("아이폰16mini", 80000);
		...
		public void ObjectRead() {
			...
			try (ObjectInputStream ois = new ObjectInputStream(new FileInputStream("product1.dat"))) {
				...
				if(obj instanceof Product) {
					...
					System.out.println("에러 : 객체 읽기 중 I/O 에러방생");
					e.printStackTrace();
				}catch (ClassNotFoundException e) {
					System.out.println("에러 : 직렬화 대상 클래스를 찾을수 없음");
					e.printStackTrace();
				}
		}
}
