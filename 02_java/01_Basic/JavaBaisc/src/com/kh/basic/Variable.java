...
public class Variable {
	...
	public static void main(String[] args) {
		...
		boolean isTrue = false;//선언과 동시에 false라는 값 대입
		isTrue = true;
		 ...
		 * 모두 정수를 저장하는 자료형, 각 자료형마다 저장할 수 있는 수의 범의가 다름.
		 * 
		 * 정수 리터럴의 기본값은 int
		...
		boolean b1 = 10 > 3; //true
		boolean b2 = 23 ==32;//false
		 ...
		 * 문자열
		 * String 
		 * 문자열 같은 경우 할당할 메모리크기가 가변적임으로 원시타입이 아닌 객체타입으로 변수를 만들어 사용.
		 * 
		 * 원래 객체를 만드는 방식
		...
		String st1 = "안녕하세요. 스프링입니다."; //참조 변수는 내용을 따로 메모리에 저장하고 따로 저장한 메모리에 주소를 저장하고 주소를 가져오는 것이다.
		System.out.println(st1);
		
		String str2;
		str2 = "누구?";
		str2 = "저는 누구가 아니라 최지원입니다.";
		str2 = null; //String의 변수는 참조변수이기 때문에 기본값이 null이고 빈값을 표현하기위해 null을 대입할 수잇다.
		...
		// \\ : 백슬래쉬(\)
		// 문자 자체로 출력
		System.out.println("경로 : C:\\ORACLE SQL\\workspce\\SQL\\과제 파일\\WORKBOOK_ ");
		...
		final int MY_AGE;
		MY_AGE=200;
		//MY_AGE=202; --값을 한번 초기화하면 더 이상 변경할 수 없음.
		System.out.println(MY_AGE);
		
		
		/*
		 * 프로그래밍에서 이름 짓는 방식
		 * 카멜케이스 : 단어를 나열하며 이름을 짓되 두번쨰단어부터는 단어의 첫글짜를 대문자로 시작
		 * ex) userName, totalPrice
		 * 
		 * 파스칼케이스 : 카멜케이스에서 첫글자도 대문자로 작성(자바에서는 클래스명에 사용)
		 * ex) UserName, TotalPrice
		 * 
		 * 스네이크케이스 : 문자와 문자사이에 언더스코어(_)를 통해서 이어주는 것
		 * ex) user_name, total_price
		 * 
		 * 대문자스네이크케이스 : 스네이크케이스 + 모두 대문자 (대부분 언어에서 상수명)
		 * ex) USER_NAME
		 * 
		 * 케밥케이스 : 단어를 소문자로 작성하되 문자와 문자사이를 하이픈(-)으로 이어주는것(html속성, url, 설정파일,등)
		 * ex) user-name, total-price
		 */
	}
