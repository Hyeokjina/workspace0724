...
public class Input {
	...
	public static void main(String[] args) {
		...
		/*String str1,str2;
		//.next()
		//문자열 입력받는 함수(공백 전까지)값
		//입력받은 값을 특정 변수에 바로 대입;
		System.out.print("입력 :");
		str1 = sc.next();
		str2 = sc.next();
		
		System.out.println("바로직전 입력한값 :" + sc.next());
		System.out.println("str1에 입력한값 :" + str1);
		System.out.println("str2에 입력한값 :" + str2);
		*/
		
		//.nextline()
		//문자열을 입력받는 함수(공백을 포함한 한줄, 개행문자까지)
		System.out.print("이름 :");
		String name = sc.nextLine();
		
		System.out.print("나이 :");
		int age = sc.nextInt(); //정수를 입력받고자 할 때
		sc.nextLine(); //버퍼에 남아있는 nextIn()를 입력할 때 발생한 \n(엔터값)을 비워주는 코드
		...
		//문자이외에 원시타입을 입력받을 때
		//.next타입()
		// nextInt(), nestDouble(), nextBoolean()....
		
		sc.close(); //Scanner자원을 다쓰고 반납 *한번 반납하면 다시 사용x
		//sc = new scaneer(System.in); 에러가 발생함
	}
