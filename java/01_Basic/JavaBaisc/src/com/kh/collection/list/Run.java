...
public class Run {
	...
	public static void main(String[] args) {
		...
		list.add(new String("이무진")); //여러타입을 저장할 수있음
		list.add(new Human("이창섭",30));
		list.add(new Human("임재현",27));
		
		System.out.println(list);
		
		//지정된 크기보다 더 많이 추가해도 에라가 발생하지 않음. 장점1 크기제약x
		//다양한 타입의 데이터를 담을 수 있음 장점2 여러타입 보관가능
		
		//2.add(int index, E e) : 직접인덱스를 지정해서 해당위치에 데이터를 추가
		...
		//4.remove(E e) : 객체를 직접 지정해서 삭제(사용을 위해서는 객체의 eaquals를 적절하게 오보라이딩 해야한다.
		list.remove("이무진");
		list.remove(new Human("임재현",27));
		System.out.println(list);
		
		//5.set.(int index, E e) : 해당 인덱스의 값을 전달받은 e객체로 덮어씌움
		list.set(0, new Human("임재현,27", 0));
		System.out.println(list);
		...
		//8. addAll(Collection c) : 컬렉션을 통째로 뒤에 추가 할수있음
		...
		//검색 search(E e) 데이터 검색 후 존재하면 위치반환(위에서부터 센다	) -> equals를 사용하여 배교검색
