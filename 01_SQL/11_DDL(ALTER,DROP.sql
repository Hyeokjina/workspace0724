/*
    ...
    <ALTER>
    객체를 변경하는 구문
    기존에 생성된 객체(테이블, 컬럼, 제약조건 등)의 구조를 변경(수정, 추가, 제거, 이름변경)을 할 때 사용.
    
    ALTER 변경할 객체이름 변경할 내용;
    
    ALTER TABLE 테이블 변경할 내용
...
--1_3)컬럼 삭제(DROP COLUNM) : DROP COLUMN 삭제하고자하는 컬럼
...
--------------------------------------------------------------------------------
/*
    2) 제약 조건 추가/삭제(수정은 삭제하고 다시 추가하면 수정)
...
--테이블 삭제
--DROP TABLE 테이블명;
--어딘가에 참조되어있는 부모테이블은 함부로 삭제가 되지않는다.
--만약지우려고 한다면
--1. 자식테이블의 데이터 또는 테이블자체를 먼저 전부 삭제
--2. 그냥 부모테이블만 삭제하는데 제약 조건까지 삭제
...
ALTER TABLE DEPT_COPY RENAME CONSTRAINT SYS_C008716 TO DEPT_ID_NN;

--3_3) 테이블명도 변경 : RENAME TO 변경할 테이블명
ALTER TABLE DEPT_COPY RENAME TO DEPT_COPY1;
