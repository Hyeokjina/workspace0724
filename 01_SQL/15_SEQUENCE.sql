...
/*
    ...
    [START WITH 시작값] -> 처음 발생시킬 시작값을 지정(기본값1)
    [INCREMENT BY 증가값] -> 증가값을 지정할 수 있음(기본값1)
    [MAXVALUE 최대값] -> 최대값 지정(기본값이 매우 큼)
    [MINCALUE 최소값] -> 최소값 지정(기본값1)
    [CYCLE | NO CYCLE] -> 순환여부(NOCYCLE)
    [CACHE | NO CACHE] -> 캐시메모리 사용 여부
    
    *캐시 메모리 : 미리 일정량의 값들을 생성해서 메모리에 준비 -> 속도 향상
    예) CACHE 20 -> 시퀀스를 미리 20개 만들어두고 필요할 때 꺼내 씀
*/

CREATE SEQUENCE SEQ_TEST;

--현재 계정이 소유한 시퀀드르이 목록 확인
...
/*
    ...
    CURRVAL는 최최 사용시 오류 발생 -> NEXTVAL를 한번이라도 호출해야 사용가능
*/
...
/*
    ...
    [MINCALUE 최소값] -> 최소값 지정(기본값1)
    [CYCLE | NO CYCLE] -> 순환여부(NOCYCLE)
    [CACHE 바이트 수 | NO CACHE] -> 캐시메모리 사용 여부
