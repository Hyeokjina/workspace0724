...
/*
    *서브쿼리 구문
    서브쿼리를 수행한 결과값이 몇행, 몇일로 나오느냐에 따라서 분류
...
--1)전 직원의 평균급여보다 급여를 더 적게 받는 사원드의 사원명, 지급코드, 급여조회
...
--3) 노옹철 사원의 급여보다 급여를 많이 받는 사람들의 사번,이름, 부서명, 급여를 조회
...
/*
    ...
    비교대상 >,< ANY(~값) : 여러개의 결과값 중 한개라로 비교연산을 만족하는가?
    비교대상 >,< ALL(~값) : 여러개의 결과값 모두가 비교연산을 만족하는가?
    위 3가지 연산자와 함께 사용하는 경우가 많음
*/

--1) 유재식 또는 윤은혜 사원과 같은 직급인 사원들의 사번, 사원명, 직급코드,급여 조회
SELECT EMP_ID, EMP_NAME, JOB_CODE, SALARY
FROM EMPLOYEE
...
SELECT SALARY
                ...
                JOIN JOB USING(JOB_CODE)
                WHERE JOB_NAME = '과장';
--ANY를 사용(과장들과 비교해 한명이라도 급여가 높을때)
SELECT EMP_ID,EMP_NAME, JOB_NAME,SALARY
FROM EMPLOYEE
JOIN JOB USING (JOB_CODE)
WHERE JOB_NAME='대리'
...
/*
    3.다중열 서브쿼리
    결과값은 한 행이지만 나열된 컬럼수가 여러개일 겨우(두개 이상)
*/
...
--다중열 서브쿼리
--튜플 비교 :  두개 이상의 열(칼럼)을 조합해서 비교하는 방식
SELECT EMP_NAME, DEPT_CODE, JOB_CODE,HIRE_DATE
FROM EMPLOYEE
...
/*
   4.다중행 다중열 서브쿼리
   서브쿼리의 조회 결과값이 여러행 여러열 일 경우
*/
--1) 각 직급별 최소급여를 받는 사원조회 (사번, 사원명, 지급코드, 급여)
--> 사원의 사번, 사원명, 직급코드, 급여
SELECT EMP_ID,EMP_NAME,JOB_CODE,SALARY
FROM EMPLOYEE;
...
-- 각 직급별 최소급여를 받는 사원조회 (사번, 사원명, 지급코드, 급여)
SELECT EMP_ID,EMP_NAME,JOB_CODE,SALARY
FROM EMPLOYEE
...
--각 부서별 급여 합계가 전체 급여 총 합의 20프로 보다 많은 부서의 부서명, 부서별 급여 합계 조회
...
--> OREDR BY 절이 수행된 결과를 가지고 ROWNUM을 부여 : 상위 5명
SELECT ROWNUM, EMP_NAME,SALARY
