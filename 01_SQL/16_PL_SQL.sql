/*
    ...
    일반 SQL의 단점을 보완해서 변수의 정의, 조건(IF),반복(FRO,WHILE)들을 지원함.
    PL/SQL은 블록구조를 사용함
...
--------------------------------------------------------------------------------
/*
    1_2) 래퍼런스 타입 변수 : 어떤 테이블에 어떤 컬럼의 데이터타입을 참조해서 사용할지 정하는 방식
*/
DECLARE
...
--------------------------------------------------------------------------------
/*
    레퍼런스타입 변수로 EID, ENAME, JCODE,SAL, DTITLE을 선언하고 각 자료형 EMPLOYEE
    (EMP_ID, EMP_NAME, JOB_CODE, SALARY), DEPARTMENT (DEPT_TITLE)을 참조하도록
    사용자가 입력한 사번의 사번, 사원명, 직급코드, 급여 부서명 조회 후 각 벼수에 담아서 출력
*/
DECLARE
    EID     EMPLOYEE.EMP_ID%TYPE;
    ENAME   EMPLOYEE.EMP_NAME%TYPE;
...
*/
//입력받은 사번에 해당하는 사원의 사번, 사원명,급여,직급코드, 보너스 조회
...
--각 테이블에서 ENP_ID, EMP_NAME, DEPT_TITLE, NATIONAL_CODE를 참조해라
--일반변수 TEAM을 문자열 타입으로 생성하고 사용자가 입력한 사원정보를 가져와서
...
BEGIN
    ...
    END IF;
    DBMS_OUTPUT.PUT_LINE('당신의 점수는 : ' || SCORE || '점 이고 등급은 : ' || GRADE || '입니다.');
END;
/
--------------------------------------------------------------------------------
/*
...
--I에 10부터 1로 역순으로 부여해서 각각 반복문내의 코드를 실행함.
BEGIN 
    FOR I IN 1..10
    LOOP
        DBMS_OUTPUT.PUT_LINE(I);
...
/*
    ...
    *시스템 예외 (오라클이 미리 설정해둔 예외)
    ...
    -ZERO DIVIDE : 0으로 나눌경우
    -DUP_VAL_ON_INDEX : UNIQUE제약조건 위해
    ...
*/
