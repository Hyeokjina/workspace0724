...
import { Bot, Pencil, UserCircle, Activity, Calendar, ChevronLeft, X, Mail, Hash, Briefcase, Building, MapPin, Phone, Target, ClipboardList, Stethoscope, Gift, FileText, Upload, CheckCircle2, AlertCircle, Lock, HelpCircle, Camera, BellRing, ChevronRight, Download, CheckSquare, BrainCircuit, ArrowRight, Search, RotateCcw } from 'lucide-react';
                    ...
                    {isCreator && tasks.length > 0 && (
                        <div className="mt-8 pt-8 border-t border-gray-100 animate-[fadeIn_0.2s_ease-out]">
                            ...
                            <div className="bg-white border border-gray-200 rounded-xl overflow-hidden">
                                {tasks.map(task => (
                                    <div key={task.id} className="flex items-center justify-between p-4 border-b border-gray-100 last:border-0 hover:bg-gray-50 transition-colors">
                                        <span className={`text-sm ${task.status === '완료됨' ? 'text-gray-400 line-through' : 'text-gray-800 font-medium'}`}>{task.title}</span>
                                        <span className={`text-[10px] px-2 py-0.5 rounded border ${
                                            task.status === '완료됨' ? 'bg-green-50 text-green-700 border-green-200' : 'bg-yellow-50 text-yellow-700 border-yellow-200'
                                        }`}>
                                            {task.status}
                                        </span>
                                    </div>
              ...
              {isCreator && !readOnly && (
                  <div className="bg-white rounded-xl p-6 border border-gray-200 shadow-sm text-center">
                      <div className="w-12 h-12 bg-green-50 rounded-full flex items-center justify-center mx-auto mb-4">
                          <BrainCircuit size={24} className="text-[#00C471]" />
                      </div>
                      <h3 className="font-bold text-gray-900 mb-2">마음 건강 체크</h3>
                      <p className="text-sm text-gray-500 mb-6">
                          정기적인 자가진단으로<br/>나의 마음 상태를 점검해보세요.
                      </p>
                      <button 
                      ...
                      >
                          <ClipboardList size={16} /> 우울증 자가진단 시작
                      </button>
                  </div>
              )}
