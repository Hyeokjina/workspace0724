/*
   <JOIN>
   두개 이상의 테이블에서 에티어를 조회하고자 할때 사용되는 구문
   조회 결과는 하나의 결과 (RESULT SET)을 반환한다.
   
   관계형 데이터버이스에서는 최소한에 데이터를 각각의 테이블에 담고 있음
...
/*
     ...
     연결시키는 컬럼의 값이 일치하는 행들만 조회(일치하는 겂이 없는 행은 조회 제외)
*/
...
--JOIN절에 같이 조인하고자하는 테이블을 기술 + 매칭시킬 컬럼에대한 조건
--[INNER] JOIN [USING/ON]
...
--2.부서와 지역테이블을 참고하여 전체 부서의 부서코드, 지역코드, 지역명 조회
SELECT DEPT_ID,LOCAL_CODE,LOCAL_NAME
FROM DEPARTMENT
...
/*
      2.외부조인(OUT JOIN) / 포괄조인
      두 테이블간의 JOIN시 일치하지않은 행도 포함시켜 조회가능
...
--1) LEFT [OUTER] JOIN : 두 테이블 중 왼편에 기술되 테이블을 기준으로 JOIN **이걸 가장 많이 사용한다.
SELECT EMP_NAME, DEPT_TITLE, SALARY, SALARY*12
FROM EMPLOYEE
...
/*
    3.비등가 조인
    메칭시킬 컬럼에 대한 조건 작성시 '='을 사용하지 않는 조인문
    
    범의(수치적 범위, 기간적 범위)를 기반으로 조인을 구현할 때 사용
*/
--사원명, 급여, 급여레벨
...
--사번, 사원명, 부서명, 지역명 조회
SELECT EMP_ID,EMP_NAME,LOCAL_NAME
FROM EMPLOYEE
JOIN DEPARTMENT ON (DEPT_ID=DEPT_CODE)
JOIN LOCATION ON (LOCATION_ID=LOCAL_CODE);
...
SELECT EMP_ID,EMP_NAME,DEPT_TITLE,JOB_NAME,LOCAL_NAME,NATIONAL_CODE,
       SAL_LEVEL
FROM EMPLOYEE
JOIN DEPARTMENT ON (DEPT_ID=DEPT_CODE)
JOIN JOB USING (JOB_CODE)
...
/* 
    내추럴 조인(NATURAL JOIN)
    두 테이블간에 공통된 컬럼 이름을 기준으로 정해서 자동으로 조인하는 방식
