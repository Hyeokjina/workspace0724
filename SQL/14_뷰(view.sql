/*
    ...
    자주 사용하는 SELECT문을 하나의 가상 테이블처럼 저장해 좋은 객체.
    실제 테이터를 저장하지 않고, 정의된 SELECT문을 실행해서 데이터를 조회.
    복잡한 조인쿼리를 매번 작성하지 않고 간단하게 재사용할 수 있음.
*/

--한국에서 근무하는 사원들의 사번, 이름, 부서명, 급여, 근무국가를 조회
...
--러시아에서 근무하는 사원들의 사번, 이름, 부서명, 급여, 근무국가를 조회
SELECT EMP_ID, EMP_NAME, DEPT_TITLE,SALARY,NATIONAL_NAME
FROM EMPLOYEE
JOIN DEPARTMENT ON (DEPT_CODE=DEPT_ID)
JOIN LOCATION ON (LOCATION_ID= LOCAL_CODE)
...
--일본에서 근무하는 사원들의 사번, 이름, 부서명, 급여, 근무국가를 조회
SELECT EMP_ID, EMP_NAME, DEPT_TITLE,SALARY,NATIONAL_NAME
FROM EMPLOYEE
JOIN DEPARTMENT ON (DEPT_CODE=DEPT_ID)
JOIN LOCATION ON (LOCATION_ID= LOCAL_CODE)
...
/*
    ...
    AS 서브쿼리
*/
--GRANT CREATE VIEW TO C##SERVER; : 뷰 생성 권한(관리자계저에서 작성 후 실행)
...
--------------------------------------------------------------------------------
/*
    뷰 컬럼에 연산식,함수식이 있는 경우 무조건 별칭을 지정해야한다.
*/
...
--------------------------------------------------------------------------------
--생성된 뷰를 통해서 DML(INSERT, UPDATE,DELETE)사용이 가능함.
...
/*
    ...
    [상세 표현식]
    CREATE [OR REPLACE] [FROM | NOFORCE] VIEW 뷰명
    AS 서브쿼리
    [WITH CHECK OPTION]
    [WITH READ ONLY]
    ...
    >NOFORCE : 서브쿼리에 기술된 테이블이 존재 할 때만 뷰 생성 가능.(기본값)
...
AS (SELECT TCODE, TANEM, TCONTENT
    FROM TT);
DROP VIEW VW_EMP;
--서브쿼리에 기술된 테이브링 존재하지 않아도 뷰는 우선 생성된다.
