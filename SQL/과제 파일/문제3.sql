--1.학생이름과 주소지를 표시하시오 단, 출력 해데는"학생이름","주소지"로 하고 정렬은 오름차순
...
FROM TB_STUDENT
WHERE ABSENCE_YN='Y'
ORDER BY SUBSTR(STUDENT_SSN,1,6) ASC;
...
--4.현재 법학과 교수중 가장 나이가 많은 사람부터 이름을 조회
...
--5.2004년 2학기에 'C3118100'과목을 수강한 학생들의 학점 조회 학점순
...
--9. 8번의 결과 중 '인문사회' 계열에 속한 과목에 교수 이름을 찾아라 
SELECT CLASS_NAME,PROFESSOR_NAME
FROM TB_CLASS
...
--10.'음악학과'학생들의 평점을 구하라 음악학과 학생들의 학번 학생 이름 전체평점을 출력
...
--11.학번이 A313047인 학생이 학교에 나오고 있지 않다. 지도 교수에게 내용을 전달하기 위하
--학과 이름, 학생 이름, 지도교수 이름을 출력
...
--12. 2007년도에 '인간관계론' 과목을 수강한 학생을 찾아 학생이름과 수강학기를 조회
...
--13.예체능 계열 과목 담당 교수를 한 명도 배정받지 못한 과목을 찾아 그 과목 이름과 학과
--이름 출력
SELECT CLASS_NAME, DEPARTMENT_NAME
FROM TB_CLASS
...
--14.춘 기술대학교 서반아어학과 학생들의 지도교수를 게시하고자한다. 학생이름과 지도교수 이름을 찾고
--만일 지도 교수가 없는 학생을 경우 "지도교수미지정"으로 출력
...
--15.휴학생이 아닌 학생 중 평점이 4.0이상인 학생의 학번, 이름 ,학과이름, 평점을 출력
SELECT   STUDENT_NO,STUDENT_NAME,DEPARTMENT_NAME,
...
--18.국어국문학과에서 총 평점이 가장 높은 학생의 이름과 학번을 조회
...
SELECT STUDENT_NO, STUDENT_NAME, AVG(POINT)
      ...
      GROUP BY STUDENT_NO, STUDENT_NAME
      ORDER BY AVG(POINT) DESC;
--19.춘 기술대학교의 "환경조경학과"가 속한 같은 계열 학과들의 학과 전공과목 평점을 파악 하기
--위한 적절한 SQL문을 만들어라 단 출력해더는 계열 학과명, 전공평점으로 표시하며 평점을 소수점 한자리까지만 반올림
...
WHERE CATEGORY=(SELECT CATEGORY
                FROM tb_department
                WHERE DEPARTMENT_NAME='환경조경학과')
GROUP BY  DEPARTMENT_NAME ,DEPARTMENT_NAME;
