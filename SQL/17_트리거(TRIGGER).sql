/*
    ...
    트리거 특징
    - 사용자가 명시적으로 실행하지 않아도 자동실행됨.
    - 데이터 무결성, 로깅, 저동처리 등에 유용.
    ...
    *적용대상(어디에 적용하는가?)
    -문장 트리거 : 이벤트가 발생한 SQL문에 대해 딱 한번만 트리거실행(FRO EACH ROW 없음)
...
--------------------------------------------------------------------------------
--입출고에 대한 데이터 기록(INSERT)될때마다 해당 상품에대한 재고 수량을 매번 수정(UPDATE)
--상품테이블(제고), 입출고기록테이블
...
-- 샘플 상품 데이터 입력
INSERT INTO TB_PRODUCT VALUES(SEQ_PCODE.NEXTVAL, '갤럭시S25', '삼성', 0);
INSERT INTO TB_PRODUCT VALUES(SEQ_PCODE.NEXTVAL, '아이폰', '애플', 0);
INSERT INTO TB_PRODUCT VALUES(SEQ_PCODE.NEXTVAL, '대륙폰', '샤오미', 0);

--2.상품 입출고 상세 이력 테이블 생성(TB_PRODEAIL)
--어떤 상품이 어떤 날짜에 몇개가 입 출고 되는지 기록
CREATE TABLE TB_PRODETAIL (
    DETAIL_ID NUMBER PRIMARY KEY,               -- 입출고 이력 번호
    PCODE     NUMBER REFERENCES TB_PRODUCT,     -- 상품번호 (외래키)
    PDATE     DATE NOT NULL,                    -- 입출고 날짜
    AMOUNT    NUMBER NOT NULL,                  -- 입출고 수량
    STATUS    CHAR(3) CHECK(STATUS IN ('IN', 'OUT'))  -- 입출고 구분
);
...
VALUES (SEQ_DETAIL_ID.NEXTVAL, 200, SYSDATE, 10, 'IN');
...
-- 아이폰 출고 3개
INSERT INTO TB_PRODETAIL
VALUES (SEQ_DETAIL_ID.NEXTVAL, 205, SYSDATE, 3, 'OUT');

-- 대륙폰 입고 20개
INSERT INTO TB_PRODETAIL
VALUES (SEQ_DETAIL_ID.NEXTVAL, 210, SYSDATE, 20, 'IN');
