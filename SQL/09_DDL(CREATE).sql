--------------------------5일차 시작---------------------------------------------
/*
    DDL(Data Definiton Language) : 테이터 정의어
    데이터 베이스에서 사용하는 SQL명령어 중 데이터를 저장할 객체의 구조를 정의하거나 변경하는 역활을 한다.
...
/*
    ...
    *자료형
     - 숫자(NUMBER)
     - 문자(CHAR(바이트크기) | VARCHAR2(바이트크기)) -> 반드시 크기를 지정해야함
        - CHAR : 최대 2000바이트까지 지정가능 / 고정길이 (고정된 글자 수의 데이터가 담길 경우)
        - VARCHAR2 : 최대 4000바이트까지 지정가능 / 가변길이(몇글자의 데이터가 들어올지 확신할 수없는 경우)
    -날짜(DATE)
*/
...
/*
    *NOT NULL
    해당 컬럼에 반드시 값이 존재해야하만 하는 경우 (즉, 절대 NULL이 들어오면 안되는 경우)
...
CREATE TABLE MEMBER (             -----NO,ID,PWD,NAME에 NOT NULL 추가
...
INSERT INTO MEMBER 
VALUES(1,NULL,'PASS01','홍길동',NULL,'010-1111-2222',NULL,NULL);

INSERT INTO MEMBER 
VALUES(1,'USER01','PASS01','홍길동',NULL,'010-1111-2222',NULL,NULL);
--아이디가 중복되었음에도 불구하고 잘 추가가 됨. 

--------------------------------------------------------------------------------
/*
   *UNIQUE 제약조건
   해당 컬럼에 중복값이 들어가서는 안될경우 사용된다.
   컬럼값에 중복값을 제한하는 제약 조건.
   삽입/수정시 기존에 있는 데이터 값중 중복값이 있을 경우 오류를 발생시킴
*/

DROP TABLE MEMBER;--테이블 삭제

CREATE TABLE MEMBER (          -----ID와 EMAIL에 UNIQUE 추가
    MEM_NO NUMBER NOT NULL,
...
CREATE TABLE MEMBER (                           -----각각 제약조건에 제약조건명 추가
    ...
    EMAIL VARCHAR2(50),
    CREATE_AT DATE,
    CONSTRAINT EMAIL UNIQUE(EMAIL)--테이블 레벨 부여방식
);
...
INSERT INTO MEMBER 
VALUES(1,'USER02','PASS01','홍길동','왜','010-1111-2222',NULL,NULL);
...
CREATE TABLE MEMBER(                 -----GENDER에 남과 여 CHECK조건문 추가
    MEM_NO NUMBER CONSTRAINT MEMNO_NT NOT NULL,
...
--단 , NULL은 값이 없다는 뜻이기 때문에 CHECK제약 조건

--------------------------------------------------------------------------------
/*
   PRIMARY KEY(기본 키) 제약조건
   테이블에서 각 행(ROW)를 식별하기 위해 사용될 컬럼에 부여하는 제약조건(식별자 역활)
   
   EX) 회원번호, 학번, 부서코드, 주민번호, 택배운송장번호....
   기본키 -> 중복금지(UNIQUE, + NULL 금지(NOT NULL)
   식별, 무결, 연결
...
CREATE TABLE MEMBER(
    ...
    CREATE_AT DATE,
    CONSTRAINT EMAIL UNIQUE(EMAIL)--테이블 레벨 부여방식
    --  CONSTRAINT MEMNO_PK PRIMARY KEY(MEM_NO)--테이블 레벨 부여방식
);
...
INSERT INTO MEMBER
VALUES(1,'USER02','PASS02','홍길동','남','010-1111-3333','AAA@NAVER.COM','25/07/30');
--기본키에 중복된 값을 담으려고할 때 (UNIQUE 제약조건 위반)

INSERT INTO MEMBER
VALUES(NULL,'USER02','PASS02','홍길동','남','010-1111-3333','AAA@NAVER.COM','25/07/30');
--기본키에 NULL 값을 담으려고할 때 (NO NULL 제약조건 위반)

INSERT INTO MEMBER
VALUES(2,'USER02','PASS02','홍길동','남','010-1111-3333','BBB@NAVER.COM','25/07/30');

INSERT INTO MEMBER
VALUES(3,'USER03','PASS03','홍길삼','남','010-1111-3333','CCC@NAVER.COM','25/07/30');

INSERT INTO MEMBER
VALUES(4,'USER04','PASS04','홍길사','남','010-1111-3333','DDD@NAVER.COM','25/07/30');
...
INSERT INTO MEM
VALUES(2,'USER2','PASS2','홍길동',NULL,NULL,NULL,'25/01/01',10);

INSERT INTO MEM
VALUES(3,'USER3','PASS3','홍길동',NULL,NULL,NULL,'25/01/01',20);

INSERT INTO MEM
VALUES(4,'USER4','PASS4','홍길동',NULL,NULL,NULL,'25/01/01',40);

INSERT INTO MEM
VALUES(5,'USER5','PASS5','홍길동',NULL,NULL,NULL,'25/01/01',50);
--50이들어가는 모순이 발생
...
/*
     ...
     ->다른 테이블을 참조한다고 표현
     ->대부분의 경우는 FOREIGN KEY를 통해서 테이블간의 관계가 활성화 된다.
     
     >컬럼레벨방식
     컬럼명 자료형 REFERENCES 참조할테이블명 [참조할컬럼명]
                 
     >테이블레벨방식
     FROEIGN KEY(컬럼명) REFERENCES 참조할테이블명[참조할컬럼명]
*/
...
--유효하지않은 회원등급번호는 정상적으로 INSERT 할수 없음
INSERT INTO MEM
VALUES(5,'USER5','PASS5','홍길동',NULL,NULL,NULL,'25/01/01',50);

SELECT * FROM MEM;
--MEN_GRADE(부모테이블) -> ----|- MEN(자식테이블)
--부모테이블 : 외래키로 참조되는 테이블
...
DELETE FROM MEM_GRADE
WHERE GRADE_CODE =30;
--MEM테이블에서 참조하고있지 않은 30DML GRADE_CODE를 가진 데이터는 삭제가 가능함.
...
--------------------------------------------------------------------------------
/*
   자식테이블 생성시 외래키 제약조건 부여할 대 삭제옵션을 함께 지정가능.
   *삭제옵션 : 부모테이블의 데이터 삭제시 그 데이터를 참조하고있는 자식테이블의 데이터를 어떻게 처리할 것인가?
   ...
   -ON DELETE CASCADE : 부모데이터 삭제시 해당 데이터를 사용하고 있는 자식데이터 모두 삭제.
...
DELETE FROM MEM_GRADE
WHERE GRADE_CODE IN (20,40);
--MEM_GRADE의 PK 20과 40 데이터가 삭제되면 SET NULL 옵션에 따라 사라지고
--GRADE_CODE 10을 참조하던 데이터는 NULL이 된다.
...
DELETE FROM MEM_GRADE
WHERE GRADE_CODE =10;
--MEM_GRADE의 PK 10 데이터가 삭제되면 CASCADE 옵션에 따라 자식데이터 중
--GRADE_CODE 10을 참조하던 데이터는 모두 함께 삭제된다.
...
--INSERT IMTO 테이블명(컬럼1, 컬럼2) VALUES(값1, 값2);
...
ALTER USER C##KH DEFAULT TABLESPACE USERS QUOTA UNLIMITED ON USER;
...
--CREATE TABEL 테이블명 AS SELECT문 
--AS 뒤에 오는 SELECT문의 결과를 기반으로 테이블을 생서하겠다.
...
--------------------------------------------------------------------------------
/*
    테이블이 생성된 후에 제약 조건을 수정하는 방법
    ALTER TABLE 테이블명 변경할 내용
    
    제약조건 추가 방법
    -PRIMARY KEY : ALTER TABLE 테이블명 [CONSTRATINT 제약조건명] ADD PRICARY KEY(컬럼명);
    -FOREIGN KEY : ALTER TABLE 테이블명 [CONSTRATINT 제약조건명] ADD FOREIGN KEY(컬럼명) REFERENCES 참조할테이블명(컬럼명);
    -UNIQUE : ALTER TABLE 테이블명 [CONSTRATINT 제약조건명] ADD UNIQUE(칼럼명);
    -CHECK : ALTER TABLE 테이블명 [CONSTRATINT 제약조건명] ADD CHECK(컬럼에대한 조건식);
